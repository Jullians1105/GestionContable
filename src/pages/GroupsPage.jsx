  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full">
        <p className="text-sm text-[#191c1e] mb-4">{message}</p>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#191c1e]">Grupos de Trabajo</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {groups.map((group) => {
          ...
          return (
            <div key={group.id} className="bg-white rounded-2xl border border-[#c3c6d7] p-5 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-[#191c1e]">{group.name}</h3>
                    {leader && <p className="text-xs text-[#434655]">Líder: {leader.name}</p>}
                  </div>
                </div>
                <div className="flex gap-1">
                  <button onClick={() => guardedManage(() => setEditGroup(group))} className="p-1.5 rounded-lg hover:bg-[#edeef0] transition">
                ...
                <div className="flex items-center gap-1">
                  <div className="flex -space-x-1.5">
                    {groupMembers.slice(0, 4).map((m) => (
                      <div key={m.id} className="w-6 h-6 rounded-full border-2 border-white flex items-center justify-center text-white text-[9px] font-bold bg-blue-500" title={m.name}>
                        {m.name[0]}
                      </div>
                    ))}
                  </div>
