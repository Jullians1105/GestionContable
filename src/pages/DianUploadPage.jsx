...
export default function DianUploadPage() {
  ...
  const dropStyle = estado === 'loading'
    ? `${dropBase} border-[#c3c6d7] bg-white opacity-60 cursor-not-allowed`
    : isDragOver
      ? `${dropBase} border-[#003B43] bg-[#003B43]/10 border-solid`
      : `${dropBase} border-dashed border-[#c3c6d7] bg-white hover:border-[#003B43] hover:bg-[#E3EEEE]`
      ...
      <div className="bg-white rounded-2xl border border-[#e2e4ef] shadow-sm p-8">
        ...
        {(estado === 'idle' || estado === 'error') && (
          <>
            ...
            <div className="mb-7">
              ...
              <div className="inline-flex bg-[#f0f2f8] rounded-xl p-1 mb-3">
                <button
                  type="button"
            ...
            <div className="h-px bg-[#f0f2f8] mb-7" />
            ...
            <div>
              ...
              <div className="mt-4 flex justify-center">
                <button
                  onClick={() => inputRef.current?.click()}
                  className="flex items-center gap-2 px-5 py-2 rounded-xl text-sm font-semibold border border-[#d1d5db] text-[#434655] hover:bg-[#f3f4f6] transition active:scale-[0.97]"
                >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-3 mt-3">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-[#E5A70C] text-[#20160A]"
                    >
                      Sí, es la misma empresa
                    </button>
                    <button
                      onClick={reintentar}
                      className="px-3 py-1.5 rounded-lg text-xs font-semibold border border-amber-300 text-amber-800"
                    >
            ...
            {estado === 'error' && errorMsg && !confirmacionPendiente && (
              <div className="mt-5 flex items-start gap-3 p-4 rounded-xl bg-red-50 border border-red-200">
                ...
                <div className="flex-1">
                  ...
                  {archivoNombre && (
                    <p className="text-xs text-red-500 mt-0.5">{archivoNombre}</p>
                  )}
                </div>
                <button
                  onClick={reintentar}
        ...
        {estado === 'loading' && (
          <div className="flex flex-col items-center gap-5 py-12">
            {/* Spinner */}
            <svg
              className="w-12 h-12 text-[#E5A70C] animate-spin"
              viewBox="0 0 24 24"
              fill="none"
              aria-hidden
            >
            ...
            <div className="text-center">
              <p className="text-sm font-semibold text-[#434655]">Procesando…</p>
              <p className="text-xs text-[#9ca3af] mt-1 max-w-xs truncate">{archivoNombre}</p>
            </div>
          </div>
        )}
        ...
        {estado === 'success' && (
          <div className="flex flex-col items-center gap-4 py-12">
            ...
            <p className="text-xs text-[#9ca3af]">Redirigiendo a clasificación…</p>
          </div>
        )}
      </div>
    </div>
