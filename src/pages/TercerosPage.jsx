  ...
  return (
    <div>
      ...
      >
        ...
        <span className="material-symbols-outlined text-3xl" style={{ color: archivos.length > 0 ? '#16a34a' : '#E5A70C' }}>
      ...
      {archivos.length > 0 && (
        <div className="mt-3 max-h-48 overflow-y-auto rounded-xl border border-[#e2e4ef]">
          {archivos.map((f, i) => (
            <div
              key={`${f.name}-${i}`}
              className={`flex items-center justify-between gap-2 px-3 py-2 text-xs ${i % 2 === 1 ? 'bg-[#f8f9fc]' : ''}`}
            >
              <span className="truncate text-[#434655]">{f.name}</span>
              <button
                type="button"
                onClick={() => onQuitar(i)}
          ...
          <button
            type="button"
            onClick={onLimpiar}
            className="w-full px-3 py-2 text-xs font-semibold text-[#6b7280] hover:bg-[#f3f4f6] transition border-t border-[#e2e4ef]"
          >
            Quitar todos
          </button>
        </div>
    ...
    <div className="max-w-[1100px] mx-auto mt-8 mb-16">
      ...
      {(estado === 'idle' || estado === 'procesando' || estado === 'error') && (
        <div className="bg-white rounded-2xl border border-[#e2e4ef] shadow-sm p-6 mb-4">
          <h2 className="text-sm font-bold text-[#191c1e] mb-1">¿Estos PDFs son de compras o de ventas?</h2>
                ...
                <button
                  ...
                  className={`relative flex flex-col items-start gap-1 rounded-2xl border-2 transition-all duration-150 px-4 py-4 text-left ${
                    checked
                      ...
                      : 'border-dashed border-[#c3c6d7] bg-white hover:border-[#003B43] hover:bg-[#E3EEEE]'
                  }`}
                >
                  <span
                  ...
                  <span className="material-symbols-outlined text-2xl" style={{ color: checked ? '#E5A70C' : '#003B43' }}>{op.icon}</span>
                  <p className="text-sm font-bold text-[#191c1e]">{op.label}</p>
                  <p className="text-xs text-[#6b7280]">{op.hint}</p>
                </button>
              )
            })}
          </div>
      ...
      {estado === 'resultado' && resultado && (
        <div>
          ...
          {resultado.omitidosNoFactura > 0 && (
            ...
            <div className="mb-4 flex items-center gap-2 rounded-xl bg-[#f0f2f8] px-4 py-2.5 text-sm text-[#6b7280]">
          ...
          <div className="bg-white rounded-2xl border border-[#e2e4ef] shadow-sm overflow-hidden overflow-x-auto">
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr>
                  <th className="bg-[#f8f9fc] text-[#434655] font-semibold border-b border-[#e2e4ef] px-3 py-2.5 text-left whitespace-nowrap">Razón social</th>
                  <th className="bg-[#f8f9fc] text-[#434655] font-semibold border-b border-[#e2e4ef] px-2 py-2.5 text-left whitespace-nowrap">NIT</th>
                  <th className="bg-[#f8f9fc] text-[#434655] font-semibold border-b border-[#e2e4ef] px-2 py-2.5 text-left whitespace-nowrap">Dirección</th>
                  <th className="bg-[#f8f9fc] text-[#434655] font-semibold border-b border-[#e2e4ef] px-2 py-2.5 text-left whitespace-nowrap">Municipio</th>
                  <th className="bg-[#f8f9fc] text-[#434655] font-semibold border-b border-[#e2e4ef] px-2 py-2.5 text-left whitespace-nowrap">Cód. DANE</th>
                  <th className="bg-[#f8f9fc] text-[#434655] font-semibold border-b border-[#e2e4ef] px-2 py-2.5 text-left whitespace-nowrap">Estado</th>
                </tr>
              </thead>
              <tbody>
                {resultado.terceros.map((t, idx) => (
                  <tr key={`${t.nit}-${idx}`} className={idx % 2 === 1 ? 'bg-[#f8f9fc]' : ''}>
                    <td className="px-3 py-2 border-b border-[#e2e4ef] text-[#191c1e]">{t.razon_social}</td>
                    <td className="px-2 py-2 border-b border-[#e2e4ef] text-[#434655] whitespace-nowrap">{t.nit}</td>
                    <td className="px-2 py-2 border-b border-[#e2e4ef] text-[#434655]">{t.direccion || '—'}</td>
                    <td className="px-2 py-2 border-b border-[#e2e4ef] text-[#434655] whitespace-nowrap">{t.municipio || '—'}</td>
                    <td className="px-2 py-2 border-b border-[#e2e4ef] whitespace-nowrap">
                      {t.pendienteDesambiguar ? (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700">Ambiguo</span>
                        ...
                        <span className="text-[#434655]">{t.codigo_departamento_dane}-{t.codigo_municipio_dane?.slice(2)}</span>
                      ) : t.municipio ? (
                        ...
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-[#f0f2f8] text-[#9ca3af]">Sin mapear</span>
                      ) : (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-red-100 text-red-700">Sin datos</span>
                      )}
                    </td>
                    <td className="px-2 py-2 border-b border-[#e2e4ef] whitespace-nowrap">
                      {t.esNuevo ? (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-green-100 text-green-700">Nuevo</span>
                      ) : t.cambios?.length > 0 ? (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-700" title="Se actualizó la información de esta empresa">Actualizado</span>
                      ) : (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-[#f0f2f8] text-[#9ca3af]">Sin cambios</span>
                      )}
                    </td>
                  </tr>
                ))}
