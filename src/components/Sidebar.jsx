...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <>
      <aside className={`fixed left-0 top-0 h-full z-50 flex transition-transform duration-300 ease-in-out ${open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>

        {/* Columna de módulos - 64px */}
        <div className="w-16 h-full flex flex-col items-center py-4 gap-1" style={{ background: '#004ac6' }}>
          <div className="mb-4">
            <img src="/logo.jpeg" alt="Logo" className="w-9 h-9 rounded-xl object-cover flex-shrink-0" />
          </div>
        ...
        {/* Sidebar de contenido - 250px */}
        <div className="w-[250px] h-full bg-white dark:bg-[#1e2030] border-r border-[#c3c6d7] dark:border-[#2e3148] flex flex-col p-4 gap-1">
          ...
          {activeModule === 'tasks' ? (
            ...
            <>
              <div className="flex items-center justify-end px-2 py-4 mb-2">
                <button onClick={onClose} className="lg:hidden p-1.5 rounded-lg hover:bg-[#edeef0] dark:hover:bg-[#252840] text-[#434655] dark:text-[#c4c8e8] transition">
                  <span className="material-symbols-outlined text-xl">close</span>
                </button>
              </div>
              <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-4">
                <span className="material-symbols-outlined text-5xl text-[#c3c6d7] dark:text-[#3e4260]">
                  {activeModuleMeta?.icon}
                </span>
                <p className="text-sm font-semibold text-[#434655] dark:text-[#c4c8e8]">
                  {activeModuleMeta?.label}
                </p>
                <p className="text-xs text-[#8890b5] dark:text-[#5a5f7a]">Próximamente</p>
              </div>
            </>
          )}
        </div>
