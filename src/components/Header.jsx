  ...
  return (
    <header className="fixed top-0 right-0 left-0 lg:left-16 h-16 z-40 bg-white border-b border-[#c3c6d7] shadow-sm flex items-center justify-between px-4 gap-4">
      ...
      <form onSubmit={handleSearch} className="relative flex-1 min-w-0 max-w-md">
        <div className="relative focus-within:ring-2 focus-within:ring-[#003B43] rounded-lg transition-all">
          ...
          <input
            ...
            className="w-full h-10 pl-10 pr-4 bg-[#f3f4f6] border-none rounded-lg text-sm text-[#191c1e] focus:outline-none"
          />
        </div>

        {seccionesAbiertas && seccionesFiltradas.length > 0 && (
          <div
            className="absolute z-50 mt-1.5 w-full bg-white border border-[#c3c6d7] rounded-xl shadow-lg overflow-hidden"
          ...
          >
            {seccionesFiltradas.map((s) => (
              <button
                ...
                className="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-left hover:bg-[#f3f4f6] transition-colors"
              >
                <span className="material-symbols-outlined text-[#8890b5] text-lg flex-shrink-0">{s.icon}</span>
                <span className="text-sm text-[#191c1e] truncate">{s.label}</span>
      ...
      <div className="flex items-center gap-2">
        {enModuloTareas && (
          <span className="hidden lg:block text-xs font-semibold px-3 py-1 bg-[#edeef0] text-[#434655] rounded-full">
        ...
        <button
          ...
          className="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg hover:bg-[#edeef0] transition text-[#434655]"
          title="Recargar"
        >
        ...
        <div className="relative">
          <button onClick={() => setUserMenuOpen((v) => !v)} className="flex items-center gap-2 rounded-lg hover:bg-[#edeef0] px-2 py-1 transition">
            <div className="text-right hidden sm:block">
              <p className="text-xs font-semibold text-[#191c1e] leading-tight">{user?.name}</p>
              <p className="text-[10px] text-[#434655]">{ROLE_LABELS[user?.role]}</p>
            </div>
          ...
          {userMenuOpen && (
            <>
              ...
              <div className="absolute top-full right-0 mt-1 z-50 bg-white rounded-xl shadow-xl border border-[#c3c6d7] min-w-[160px] overflow-hidden">
                <button
                  onClick={() => { navigate('/profile'); setUserMenuOpen(false) }}
                  className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-[#434655] hover:bg-[#edeef0] transition"
                >
