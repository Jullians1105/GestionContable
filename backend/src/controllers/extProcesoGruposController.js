...
const getGrupos = async (req, res, next) => {
  try {
    const result = await db.query('SELECT * FROM ext_proceso_grupos ORDER BY orden ASC, created_at ASC');
  ...
  try {
    ...
    if (ordenFinal === undefined || ordenFinal === null) {
      const maxResult = await db.query(
        'SELECT COALESCE(MAX(orden), -1) + 1 AS next_orden FROM ext_proceso_grupos'
      );
      ordenFinal = maxResult.rows[0].next_orden;
    }
    const id = uuidv4();
    const result = await db.query(
      `INSERT INTO ext_proceso_grupos (id, name, orden)
       VALUES ($1, $2, $3)
       RETURNING *`,
      [id, name.trim(), ordenFinal]
    );
    await auditLog(req.user.userId, 'CREATE', 'ext_proceso_grupos', id, { name, orden: ordenFinal });
...
const updateGrupo = async (req, res, next) => {
  try {
    const { id } = req.params;
    const existing = await db.query('SELECT * FROM ext_proceso_grupos WHERE id = $1', [id]);
    if (!existing.rows[0]) return res.status(404).json({ error: 'Grupo no encontrado' });

    const { name, orden } = req.body;
    const result = await db.query(
      `UPDATE ext_proceso_grupos SET
    ...
    await auditLog(req.user.userId, 'UPDATE', 'ext_proceso_grupos', id, { name, orden });
...
const deleteGrupo = async (req, res, next) => {
  try {
    const { id } = req.params;
    const result = await db.query('DELETE FROM ext_proceso_grupos WHERE id = $1 RETURNING id', [id]);
    if (!result.rows[0]) return res.status(404).json({ error: 'Grupo no encontrado' });
    await auditLog(req.user.userId, 'DELETE', 'ext_proceso_grupos', id, {});
    res.status(204).send();
  } catch (err) {
    next(err);
  }
